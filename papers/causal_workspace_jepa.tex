...
\date{Working paper --- 21 July 2026}
...
interventions in Qwen3-0.6B and a faithful small reproduction of an
action-conditioned LeWorldModel.  An FP32 autograd audit overturns an apparent
...
become semantically aligned in late layers, while preregistered hypotheses of
an exact control--predictivity boundary or a fixed directional lag fail.  A
target-encoder Intervention-JEPA fails its preregistered target effective-rank
and causal-fidelity gates across three seeds.  In the world-model track, predictive reproduction
succeeds, but circuit and workspace candidates do not replicate; a recurrent
...
\begin{enumerate}[leftmargin=*]
  ...
  \item We train a genuine EMA/stop-gradient target-encoder
  Intervention-JEPA.  It fails target effective-rank and held-out causal-fidelity gates
  in all three seeds.
  \item We reproduce a small action-conditioned LeWorldModel and directly
  audit intervention, circuit, planning, and workspace criteria.  Predictive
  gates replicate, while the proposed circuit and workspace do not.
  ...
  calibration data until numerical convergence and protected-test gates are
  satisfied.
\end{enumerate}
...
\cite{terver2025jepawms}, latent-difference objectives
...
``Generalization'' labels the registered held-out design, not generality beyond
the stated model and prompt family.
...
LLM-TARGET-IJEPA-001 & learned intervention transport & Completed negative & \level{Generalization} & H-LLM-01B/02/04 fail in 3/3 seeds. \\
LLM-POPULATION-JACOBIAN-001 & population transport & Completed positive & \level{Generalization} & Validation confirmation passes within one factual relation. \\
...
LLM-ELEMENT-LAYER-GEOMETRY-001 & layer geometry & Completed mixed & \level{Generalization} & H-LLM-08/GEO-09 pass; H-GEO-08/CROSS-03 fail. \\
...
LLM-STATE-ONESHOT-LAYER-GEOMETRY-001 & layer onset & Completed mixed & \level{Generalization} & Competence/numerics pass; registered boundary alignment fails. \\
LLM-COUNTRY-CODE-LAYER-GEOMETRY-001 & bounded directional lag & Completed mixed & \level{Generalization} & Control and semantic specificity pass; lag and cross-relation claims fail. \\
WM-LEWM-001 & small LeWorldModel reproduction & Failed registered gates & \level{Causal mediation} & Prediction replicates; full causal/circuit/workspace bundle does not. \\
WM-POPULATION-JACOBIAN-001 & recurrent population transport & Rejected numerical gate & \level{Eligibility rejection} & No protected scientific claim is admissible. \\
...
612 patches, 93.6\% change the top token.  Donor-capital transfer is
...
\midrule
Top-token change & 0.936 & all 612 patches & \level{Causal mediation} \\
Donor transfer & 0.500 & test & \level{Causal mediation} \\
Exact-JVP combined NMSE & 0.5296 & all patches & \level{Specificity} \\
Exact-JVP candidate agreement & 0.351 & all patches & \level{Specificity} \\
Quadratic combined NMSE & 0.6391 & all patches & \level{Specificity} \\
Quadratic candidate agreement & 0.743 & all patches & \level{Specificity} \\
\bottomrule
\end{tabular}
\end{table}
...
Evidence level: \level{Generalization} within one relation/model.}
\label{fig:averaging}
\end{figure}
...
\midrule
Exact local JVP & 0.737 & 0.835 & 0.300 & \level{Generalization} \\
Train population JVP & \textbf{0.354} & 0.866 & 0.533 & \level{Generalization} \\
Quadratic Taylor & 0.774 & \textbf{0.910} & \textbf{0.833} & \level{Generalization} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Layerwise semantic specificity does not imply a universal boundary}

The element, one-shot state, and country-code studies all pass their numerical
audits and exhibit semantically aligned late-layer population transport beyond
...
$1.350/1.679$ and p95 agreement $0.233/0.100$.  Semantic specificity is strong,
but the fixed control--predictivity ordering is false for these registered
prompt families and the four-layer grid on Qwen3-0.6B.
...
\midrule
Elements & H-LLM-08; H-GEO-08/09; H-CROSS-03 & Pass; fail/pass; fail & \level{Generalization} & Donor control and row specificity pass; strict inversion/conjunction fail. \\
...
States, one-shot & H-LLM-12; H-GEO-12/13; H-CROSS-05 & Fail; fail/pass; fail & \level{Generalization} & Validation population onset lags control by one grid step; test aligns. \\
Countries, one-shot & H-LLM-14; H-GEO-14/15; H-CROSS-06 & Pass; fail/pass; fail & \level{Generalization} & Population usefulness precedes 50\% donor takeover on validation; row specificity passes. \\
\bottomrule
\end{tabularx}
\end{table*}
...
layer 21.  Evidence level: \level{Generalization} for the registered negative
directional-lag decision.}
\label{fig:country-layer}
\end{figure}

\subsection{The genuine Intervention-JEPA fails}
...
latent falls below the registered floor of 8 ($6.81$--$7.28$).  More
...
($2.04\times10^{11}$ normalized MSE).  These are negative generalization
results for one architecture and prompt family, not a universal impossibility
...
\subsection{Prediction replicates; circuit and workspace do not}
...
deviation from $0.762$ to $0.862$.  This supports a bounded faithful small
reproduction; it is not an official checkpoint or benchmark reproduction.
...
competence and lack of replication prevent treating that flag as a valid
...
candidate passed the registered workspace tests in the studied models and
controls.  We do not claim that JEPAs in general lack workspace-like
representations.
...
\midrule
Predictive reproduction & 3/3 & Pass & \level{Generalization} \\
Planning-specificity flag & 2/3 & Mixed & \level{Specificity} \\
Hidden latent+decoded mediation & 1/3 & Reject & \level{Causal mediation} \\
Raw restricted-circuit flag & 1/3 & Ineligible/reject & \level{Below target L5} \\
Competent planner & 0/3 & Ineligible & \level{Eligibility only} \\
Workspace proxy & 0/3 & Null & \level{Specificity} \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Action-path cancellation is a calibration-stage candidate}
...
Median horizon-four cancellation ratios are $2.589$, $5.272$, and $1.174$ for
...
$0.415/0.483/0.863$, exceeding within-action-pair null p95 by only
$0.045/0.027/0.022$.  Horizon-four versus horizon-one cancellation
...
\label{tab:path}
\begin{tabularx}{\textwidth}{@{}p{1.1cm}p{1.1cm}p{1.55cm}p{1.55cm}p{1.45cm}p{1.55cm}p{1.6cm}Y@{}}
\toprule
Seed & $h$ & Median $\kappa$ & Median local error & Spearman & Null p95 & Numerical status & Evidence level \\
\midrule
101 & 4 & 2.589 & 2.023 & 0.415 & 0.369 & No (v1) & \level{Availability} \\
103 & 4 & 5.272 & 1.389 & 0.483 & 0.456 & No (v1) & \level{Availability} \\
107 & 4 & 1.174 & 0.972 & 0.863 & 0.840 & Yes (v1) & \level{Availability} \\
all & 1/4 & --- & --- & --- & --- & V2 pending & \level{Availability} \\
\bottomrule
\end{tabularx}
\end{table*}

The provisional interpretation is narrow: finite action effects may follow a
curved, cancelling decoded path for some model/context pairs, making a single
local vector insufficient.  This becomes a testable scientific claim only if
high-resolution quadrature converges, a prospective rule is frozen, and the
rule succeeds on the five untouched test goals.  At the time of this draft,
protected test goals have not been touched.
...
Second, \emph{population usefulness is not ordered by one shared causal
boundary in the registered suites}.  Aligned population Jacobians become highly predictive in late
Qwen layers, but the first useful layer can precede, coincide with, or follow a
fixed donor-control threshold across relations and splits.  This falsifies the
registered fixed-boundary and directional-lag hypotheses.

Third, \emph{learned compression and shared sensitivity are not mechanism
proofs}.  The genuine Intervention-JEPA can produce a nontrivial predicted
...
\begin{itemize}[leftmargin=*]
  ...
  \item The LeWorldModel is a small synthetic reproduction.  Its weak planner
  ...
  action-path study remains validation-only calibration until protected test.
...
\texttt{\seqsplit{8edfeb336732b5f3ce7b8b210d0ba370a09e2cac}}.  The reported RTX host uses
...
\begin{itemize}[leftmargin=*]
  ...
  (\code{eb943a5});
  \item the high-resolution v2 calibration launched from \code{288f663}.
\end{itemize}
...
obey one shared control--predictivity boundary across the registered factual
families and layer grid; and the genuine target-encoder
JEPA fails its registered generalization gates.  In the physical track,
predictive reproduction succeeds while circuit and workspace claims fail, and
decoded recurrent action-path cancellation remains a falsifiable candidate
rather than a result.  Future work should prioritize direct held-out behavior,
competent planners, cross-model replication, and controls that distinguish a
causal mechanism from flexible prediction or coordinate-dependent geometry.
