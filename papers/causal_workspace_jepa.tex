...
\paragraph{August 2026 synthetic circuit falsification update.}
After the studies summarized in the abstract, we added a ground-truth
Causal-Residual Circuit Tracing audit.  The simple Stage-0 positive control
recovers its planted circuit, but the preregistered harder successor is negative
across three untouched primary seeds.  Two plants leave less than eight percent
finite-effect energy after second-order transport and are therefore ineligible
for a learned residual route under the frozen rule.  The residual-eligible plant
recovers more than 99 percent of IID and OOD residual effect with perfect
selected precision and QK-like edge recovery, but fails exhaustive planted-node
recall because redundant and cancelling pathways are omitted by a sparse greedy
reconstruction.  Equal-capacity direct-delta students also outperform
residual-target students in normalized IID and OOD error.  We retain this as a
falsification boundary: functional circuit sufficiency, exhaustive generative
graph recovery, and residual-model usefulness are distinct claims.  No Qwen,
JEPA, workspace, or state-of-the-art claim is inferred from the synthetic audit.
...
2.45.  The bound-corrected arm must average at least 0.80 success overall and
0.70 within every training seed before a bounded trained model is eligible for
mechanistic audit; the official unbounded arm remains a mandatory paired diagnostic.
Training completion or loss alone is availability evidence.  The portfolio was
launched from clean commit \code{5065108} at
\code{2026-07-21T21:53:46Z}. Seed 1 completed all 12 epochs in 10,692.85
seconds; prediction loss fell from $0.1990$ to $0.0178$, and all 13 checkpoint
identities revalidate. The portfolio stopped before seed 1000 because its
clean-worktree guard detected later repository edits. Seeds 1000/10000 and all
18 competence jobs remain unrun. These losses are operational availability,
not competence or mechanism evidence.
...
LLM-TARGET-IJEPA-DIAGNOSTIC-001 & post-hoc target geometry & Diagnostic only & \level{Availability} & Train/test rank and identity grouping clarify the failure; no hypothesis is reopened. \\
...
ordered content hash after reading every HDF5 shard back.  The outcome-blind
evaluator is now implemented in three phases: calibration, train-only plan
freezing, and protected execution.  The self-hashed plan fixes five direct
comparators, 128 structure-matched random sets, four same-denominator controls,
and the population prefix before test access.  Protected units are independently
checksummed, and execution rejects code, calibration, runtime, split-cardinality,
or plan drift.  Validation supplies task-eligibility gates only; direct decisions
use test and its paired paraphrases. Capture remains \level{availability};
H-LLM-15/16 remain undecided.  A
replacement token audit from clean commit \code{bca50e3} passes all eight gates
over 560 rows, including exact test--paraphrase factor pairing and role/split
token-ID disjointness; its episode hash is \code{96dc6320...f3be}.  This audit
executes no Qwen forward.  A separate pre-outcome milestone authorized only
the frozen capture.  That 560-row FP32 capture passes exact replay and storage
integrity but is \code{INELIGIBLE\_TASK}: clean, donor, and transfer accuracy
are zero on primary-template train, validation, and test.  The exactly paired
paraphrase reaches only $0.646/0.677$ clean/donor accuracy and fails worst-group
floors.  Token 17607 (decoded ``\texttt{ ?\textbackslash n}'') dominates the
primary outputs.  We therefore stop before calibration or ranking and leave
H-LLM-15/16 undecided.

An explicitly outcome-disclosed read-only diagnostic reuses the exact 96
test--paraphrase pairs.  Restricting predictions to the four values in each
episode yields primary versus paraphrase clean accuracy $0.375$ versus $0.771$
and donor/treated accuracy $0.281$ versus $0.885$.  The corresponding mean
correct-answer margins change from $-0.885$ to $+1.571$ and from $-0.892$ to
$+1.772$; paired four-value gains/losses are $38/0$ and $58/0$.  Thus the
format changes candidate-answer ordering as well as full-vocabulary emission.
Because the analysis is post hoc and has no internal-site intervention, it
cannot distinguish parsing, binding-state formation, readout, or output-mode
competition, select a replacement prompt, or rescue the ineligible study.

A separate prospective binding-algebra protocol freezes the finite group
$S_4$: train only primitive transpositions, then roll the same causal-state
transition over unseen double transpositions, three-cycles, four-cycles, and
action--inverse restoration cases drawn from disjoint token alphabets. Targets
are downstream causal deltas rather than complete treated residuals. Exact
JVP, population Jacobian, full cross-term quadratic transport, sequentially
relinearized JVP, ridge, bilinear, MLP, and direct state-patch controls are
mandatory. Validation first tests whether composition is nonlinear enough to
justify a learned meta-model. Execution remains unauthorized; this protocol is
not itself novelty, a causal abstraction, circuit, J-space, or workspace evidence.
...
A post-hoc replay clarifies that the $6.81$--$7.28$ ranks describe only 30
held-out rows grouped into six donor entities: train target ranks are
$17.70$--$20.39$, so this is not evidence of low-rank training collapse.
Unadjusted donor/recipient eta-squared changes from $0.002/1.000$ in clean
residuals to $0.829/0.128$ after intervention; raw causal deltas are
$0.496/0.528$, while the target latents remain donor-heavy at
$0.791$--$0.805/0.111$--$0.124$. A train-fit 64-dimensional causal-delta
PCA/ridge oracle reaches test NMSE $0.512$, versus $2.596$ for concatenated
clean/intervened state coordinates. The latter receives twice the decoder
input width, all test outcomes were already open, and neither oracle is a
learned predictor. This diagnostic motivates a prospective target ablation but
reopens no hypothesis.
...
\begin{itemize}[leftmargin=*]
  ...
  its parent design is superseded.  V2 fixes the pre-outcome confounds and its
  evaluator is implemented and synthetically tested.  Its replacement token
  audit passes, but the full capture fails the registered task-competence gate,
  so no plan or protected mediator execution is run.  It contributes no
  mediator, circuit, JEPA, or workspace evidence in this manuscript.  A
  post-hoc exact-pair diagnostic shows that template format also changes
  within-episode answer-logit ordering, but it does not localize why.
...
One training seed completed, but no planner-competence job has run; the retained
mechanistic smoke therefore remains random-weight engineering evidence. Its official MPPI planner also
