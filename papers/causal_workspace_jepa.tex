...
executes no Qwen forward.  A separate pre-outcome milestone authorizes only
the frozen capture; downstream ranking and decisions remain conditional on
capture eligibility, calibration, budget, and a committed train-only plan.
...
\begin{itemize}[leftmargin=*]
  ...
  evaluator is implemented and synthetically tested, but its replacement token
  audit, capture, plan, and protected Qwen execution are not yet run; it
  contributes no mediator, circuit, JEPA, or workspace evidence in this manuscript.
