...
Write-Host 'CRCT -> Qwen Bridge V2 (tokenizer-amended)' -ForegroundColor Cyan
...
Write-Host 'Amendment:  V2/CR-V1 immutable; V3/CR-V2 must be resolved+committed+pushed'

$pythonArgs = @(
    'scripts/run_crct_qwen_bridge_v2_suite.py',
    '--device', $Device,
...
$code = $LASTEXITCODE
if ($code -ne 0) {
    Write-Warning "Bridge V2 suite exited with code $code. Inspect/upload the newest ZIP; fail-safe diagnostics are retained."
}
exit $code
