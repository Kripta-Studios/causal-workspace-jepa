...
`QWEN-BINDING-COMPETENCE-CONFIRM-001` passed on a fresh split. Do not open
...
- `LLM-TARGET-IJEPA-001` executed from clean `3086cd4`. Its shared online encoder,
  ...
  activation fidelity and 36-answer candidate behavior. All three hypotheses failed on all seeds.
  Target effective rank missed the fixed floor, and oracle target-embedding decoding was worse than
...
- `LLM-ELEMENT-LAYER-GEOMETRY-001` ran unchanged from clean `5d8de9a` on 36 unique symbols and
  entity-disjoint 24/6/6 splits. All numerical gates passed. Donor-symbol transfer was absent at
  layers 18/21 and reached `0.60/1.00` validation plus `0.90/1.00` test at layers 24/26, so
  H-LLM-08 passed. Aligned layer-24/26 population transports also beat 256 answer-row nulls by wide
  margins, so H-GEO-09 passed. The stricter H-GEO-08 inversion failed: population/local MSE ratios
...
- `LLM-STATE-LAYER-GEOMETRY-001` ran from clean `27ebe43`. Every derivative/replay gate passed, but
  clean validation/test accuracy was `0.667/0.667`, below the preregistered `0.75`, so the study is
  `REJECTED_BEHAVIOR_GATE` and all four hypotheses are undecided. Descriptively, the score comparing
...
- `LLM-STATE-ONESHOT-LAYER-GEOMETRY-001` ran from clean `c1daa46`. Clean held-out accuracy was 1.0
  and all derivative gates passed. H-GEO-13 passed strong row-null specificity. H-GEO-12 failed:
  test control/population onset both occurred at layer 24, but validation direct control crossed at
