...
| WM-T0-001 | H-WM-02 | pending | root agent | `NOT_STARTED` | tiny JEPA | Tier 0 PointMass2D | `configs/experiments/tiny_jepa_smoke.yaml` | `python scripts/run_experiment.py --config configs/experiments/tiny_jepa_smoke.yaml` | pending | CPU VPS | 0 | < 10 min | `artifacts/metrics/tiny_jepa_smoke.json` | MSE vs mean baseline | beats mean baseline | pending | Smoke result only. | Implement Milestone 1. |
