...
| Workspace discovery | root agent | Can a compact shared causal subspace be distinguished from private channels and generic state geometry? | Conditional donor controls repair the old off-manifold confound; `WM-T0-005` now preregisters held-out task composition, direct task swaps, local-tangent controls, and three seeds. | Full reportability and published-model replication remain unavailable. | Commit the preregistered implementation, then execute once without threshold tuning. | `ACTIVE` |
