...
| GPT-2 Medium | root agent | Can intervention effects be predicted on a real transformer? | Coordinate edits were near-linear; `LLM-GPT2-003` now preregisters activation-derived contrasts and singles-to-composition transfer. | One layer, one seed, selected outputs, and constructed direction labels. | Commit and run the bounded semantic-composition study without tuning. | `ACTIVE` |
