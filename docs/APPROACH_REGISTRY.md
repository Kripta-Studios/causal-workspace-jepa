...
| Workspace discovery | root agent | Can a compact shared causal subspace be distinguished from private channels and PCA? | Positive/negative detector tests implemented; experiment preregistered. | Full criteria require richer learned model. | Run `WM-T0-003`. | `IMPLEMENTED_UNVALIDATED` |
...
| GPT-2 Medium | root agent | Can intervention effects be predicted on a real transformer? | Four-prompt causal smoke exists but is underpowered. | CPU runtime and missing strong baselines. | Run preregistered strengthened study. | `ACTIVE` |
